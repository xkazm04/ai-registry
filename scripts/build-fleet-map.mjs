 ...
 *
 * Public-safe by construction: slugs, group and context NAMES, states and counts. No paths,
 * no hosts, no evidence strings - those stay in the project's own map.
 *
...
function buildProject(slug, project, subjects) {
  ...
  if (map?.contexts) {
    for (const ctx of map.contexts) {
      ...
      groups[g].push({ name: ctx.name, subjects: governing });
    }
  }
...
function renderMd(map) {
  const out = [];
  out.push('# Fleet map', '');
  out.push(`GENERATED by \`scripts/build-fleet-map.mjs\` on ${map.generatedAt}. The map of context maps and its inverse. Slugs, names, states and counts only. Regenerate, do not edit; see [\`docs/fleet-map.md\`](../docs/fleet-map.md) for what each column means and how a candidate becomes a direction proposal.`, '');
  out.push('## Summary', '');
