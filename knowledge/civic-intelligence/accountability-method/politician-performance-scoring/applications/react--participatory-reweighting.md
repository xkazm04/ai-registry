...
- Every vector is normalized to effective weights before aggregation, so proportional
  vectors express the same lens; this alone does not deduplicate submissions; a zero-sum vector carries no lens and is
...
- **Minimum-release count** (historically named K_ANONYMITY_FLOOR): the median exists only from `K_ANONYMITY_FLOOR` valid
  ballots; below the floor the aggregate returns null and the UI shows only
  the count. This suppresses small-sample medians but does not establish
  k-anonymity or protect against repeated-release inference (`aggregate.ts:21-23`).
...
The source paths, line numbers, fixture and incident counts above remain a dated
account, not a refreshed React or consumer-runtime witness. Reverify the described
gap-rounding duel rule against individually rounded display values: 1.24 and 1.26
display differently although their raw gap rounds to zero. Check zero published
weights, missing/stale components, normalized-weight rounding residuals, default
lens equality and submission identity separately. The ~0.3 reconstruction note
is specific to the recorded default decomposition, not a bound for arbitrary
custom weights. No privacy guarantee follows from the release-count constant.
