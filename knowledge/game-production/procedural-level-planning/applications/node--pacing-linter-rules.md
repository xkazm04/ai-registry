...
`PacingFinding` (`:17-29`) is the technique's "signature plus consequence plus fix" made
...
result (`:31-37`) additionally groups findings `byRoom` for inline badges and carries
severity `counts` for summary chips — a graded report, not a pass/fail.

## The arc

`resolveArc()` (`:59-...`) prefers the document's explicit `difficultyArc` and otherwise
...
**`consecutive-combat`** (`:136-184`, severity `warning`). `COMBAT_TYPES = {combat, boss}`;
...
**`difficulty-cliff`** (`:186-...`). Threshold ±3 between arc-adjacent rooms; `delta >= 3`
...
**`monotonic-ramp`** (`:224-...`, severity `info`). Requires `arc.length >= 4`, computes
...
**`no-safe-before-boss`** (`:252-...`, severity `critical`). For each `type === 'boss'`
room, `neighborsOf()` (`:120-135`) collects both outbound and inbound edges — the comment is
...
**`unreachable-room`** (`:279-...`, severity `critical`). BFS over `buildAdjacency()` from a
