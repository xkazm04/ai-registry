...
`NBARecommendation.successProbability` is declared `number | null` (`:57`) with the
...
The factor contribution follows: at `:332`, `breakdown.successProb = moduleSuccessRate ===
null ? 0 : …`, and the renderer's `nbaFactorSegments` drops the "Success odds" segment
entirely rather than drawing an empty bar.

## The sample tiers

`NBASuccessEvidence` (`:76`) declares the three-tier ladder as a union: `'pattern'` (a
...
Choosing between competing patterns (`:314`) weights rate by a capped session count:
...
`:318` weights the specific pattern `0.7` against the module track record `0.3`, and the
...
The `runEvidence` parameter (`:161`) is passed in explicitly, and its comment names a
...
A related note at `:171` records the same class of defect for the pattern library: the
