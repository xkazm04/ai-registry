...
`EngineClass` (`statusModel.ts:160`) enumerates ten classes: `llm`, `gen2d`, `gen3d`,
`audio`, `runtime`, `tooling`, `code`, `hand-authored`, `human`, `unaudited`.
`TRUSTED_CLASSES` (`:240`) names the ones whose L0–L2 pass stands without a gate, and
`isTrustedClass` (`:250`) is the single predicate over it — the comment is explicit that
no caller re-states the membership. `ENGINE_CLASS_NOTE` (`:259`) carries the published
...
The grade cascade consumes it at `statusModel.ts:429` (`deriveCell`), in one line:
...
`inferEngine` (`:300`) carries the same fix at the resolution seam: `StepSpec.engine` is
...
`MEDIA_DELIVERABLES` (`:153`) plus the `unpowered` derivation in `deriveCell` gives the
...
`isSyntheticEntity` (`:288`) drops entities created by the headless test harness and the
