...
`SolveResult` (`goal-seek.ts:27`) carries `target`, `solvedValue`, `achievedMetric`,
`iterations`, `converged` and a human `reason`. Every non-bracketed exit routes through
one `nearest()` helper (`:58`) that returns the closest reachable endpoint with a reason
...
The degenerate flat-metric case is handled separately at `:74`, and direction is inferred
from the endpoints (`const increasing = fMax > fMin`, `:80`) rather than assumed.
Defaults: tolerance `max(1e-6, |target| · 0.005)`, `maxIterations` 48 (`:45`).

## The seeded caller

`src/lib/combat/goal-seek.ts:35` (`solveCombatTuningForTarget`) is the combat lever
...
The metric closure at `:46` clones the tuning overrides with the lever value substituted,
...
(`:44`) with a stated monotonicity claim over that band — the sweep-before-you-solve step,
