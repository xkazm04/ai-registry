...
`src/lib/gdd-compliance.ts:342` records the defaults verbatim: a module with no scan took a
...
- `MEASURED_STATUSES` (`src/lib/gdd-compliance.ts:357`) is the set `implemented | improved |
  ...
  cannot drift.
- `buildEvidence` (`:414`) returns `ComplianceEvidence` carrying `featuresTotal`,
...
- `calculateConformance` (`:526`) divides credit (`implemented + improved + partial * 0.5`)
...
`CONFIDENCE_BANDS` (`src/lib/gdd-compliance.ts:370`) maps coverage to `high` ≥ 0.75,
...
`mergeEvidence` (`:461`) sums raw counts across modules and re-derives coverage from the
