...
shared_with: []
use_when: [an agent proposes a change to its own description, designing the approval surface for self-edits, a self-document is being rewritten wholesale by the model, an autonomy mode starts resolving approvals without the person, a proposal the apply step will certainly refuse is reaching the review queue]
---
