...
Which predicate, though, is a question the surface has to be able to answer at
any instant — and a filtering table holds two candidates, not one. There is
what the controls currently display, which is a draft the user is still
composing, and there is the predicate the rows on screen were actually produced
under. Only the second describes the data, and every derived artifact — the
count, the window position, the export, the share link, the empty state's
wording — reads it. The classic breach is an export built from the controls: a
file whose contents the user never reviewed, generated by a surface that was
showing them something else. The [filtering](./techniques/filtering.md)
technique makes that separation the axis's contract.
...
- [filtering](./techniques/filtering.md) — entered versus applied, where each
  control commits, deriving the whole predicate, latest-wins under a race, and
  what a filter change does to the window and to selection.
