...
*Re-verified against the project tree at `a2ef6400e`. Every claim stands, including
the confessed ledger-vs-disk gap: nothing has been added that reconciles the
projection ledger against the vault, so the declared-or-reconciled fork is still
resolved by declaration. Nine citations moved.*
...
`mirror_write_note` (`mod.rs:335`) is the hash-gated incremental
...
(`:76-95`) pins the contract: first write → created; unchanged → skipped;
changed → written.

`mirror_execution_knowledge_for_persona` (`:410`) shows the posture
...
knowledge mirroring must never break the execution path" (`:405-409`). The
named rebuild exists as a first-class command:
`obsidian_mirror_backfill_execution_knowledge` (`:455`), invoked when
...
Push (`obsidian_brain_push_sync`, `:523`) never overwrites blind: for each
tracked entity it loads the ledgered base hash and routes through
`classify_push` (`:885`), which reads the current vault file and runs
`three_way_compare` (`conflict.rs:44`) — base vs app content vs vault
...
`mod.rs:1309`, which routes the chosen side back through the vault-path
funnel before writing).

The audit nicety the technique calls out is implemented literally:
`ThreeWayResult::ConvergedConflict` (`conflict.rs:9-30`) — both sides
...
asserting it never collapses into no-change (`:126-140`).
