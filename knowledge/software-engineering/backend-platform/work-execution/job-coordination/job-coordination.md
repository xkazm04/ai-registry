...
- **Only one at a time** — "at most one live job of this kind" is
  ...
  lifecycle.
- **Human-paced flows** —
  [wizard-flows](../../../ui-surfaces/input-and-editing/wizard-flows/wizard-flows.md) shares the resumable-
...
- **Waiting between attempts** — when a failed job is retried, the delay
  policy is [retry-backoff](../../resilience/retry-backoff/retry-backoff.md)'s; the fact
