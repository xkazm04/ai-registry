...
**Next run's declared focus (round 11).** (1) **Add the sentence the depth cell
cannot carry.** When a run's routing count is above threshold on unhomed decisions
but every home already exists, say so in the row's own words and stop letting `S0`
imply a routing failure. Round 10 hit the same shape and diagnosed it in its Depth
cell; two rounds running is a pattern, and the fix is one convention, not a new
instrument. (2) **Close `techniques_absent` in writing — eighth deferral, and this
run adds nothing new to the argument.** Round 10 already supplied the closure's
reasoning; the only thing missing is somebody writing it down. Do that before
reading this line again. (3) **Record what a confirming category-listing read
costs.** Round 9 showed a listing can overturn both maps; this round showed it
agreeing with them at all four homes. A check is only worth mandating if its
negative result is cheap, and one command per home is cheap — but that is now the
claim, and the next run should either confirm it or find the case where the read
is not cheap (a category with dozens of siblings, a bundle whose listing does not
fit one screen).
**The check the next row should make:** did the run distinguish "no new subject
because the source was thin" from "no new subject because every home already
existed", in the row itself rather than only in the source note?
