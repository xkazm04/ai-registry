...
| 1.3.0 | 2026-08-31 | aider | 1 (1,156 README vs ~9,500 LOC of core engineering mined; ~428k words in-tree markdown) | 10 | 2 | 2 (1 technique + 1 amendment, plus golden-path and roster edits) | 2 rows (0c/2e/0s) | 0 (both experiments were read-only; no product code changed, and Phase 8 confirmation for a cross-repo commit was not sought) | Declared focus **apply-under-coverage met at 1:1 for the second run running**: 2 landings, 2 rows, both with a real seam in one tree, both `better`, no `unapplied` and no `no seam`. The run's own lesson is about **Test**, not Apply: both picks came from hunting an *enumeration* and an *asymmetry* inside already-mature files, neither of which `research-map` can see - it scores two files that both "cover" a concept identically, and only opening both reveals that one models it and the other mentions it. New: the apply step generated a corpus finding of its own (a ranking field at 93.7% schema default = `unknown-is-not-a-value`), the second run in four to have the seam amend the technique rather than merely test it |
...
**Update after `anydoc` (2026-08-31).** Apply stays the weakest stage, and this run
sharpens *why* in a way the last three rows could not. The previous focus - "does 1:1
landing-to-row hold above four landings" - was tested and **failed at 1 of 7**, but the
cause is not effort. It is that **the apply stage is anti-correlated with the extract
stage's best outcome**: a run that finds a genuine hole forges a subject into empty ground,
and empty ground in the corpus tends to be empty ground in the fleet too. Five techniques
landed; exactly one had a live instance to test against, and it converted to `code`/`better`
with paired arms on the project's own gate.

So the funnel metric needs a distinction the notation does not yet draw: **unapplied
because nobody tried** and **unapplied because the fleet does not do this yet** are
different failures, and only the first is the skill's. Four of this run's six unapplied
rows are the second kind and each names what would create a seam. The next run's declared
focus: **report apply coverage over findings that HAVE a seam**, and count the seamless
ones separately, so the number stops punishing the runs that find the emptiest ground.
