...
**Next run's declared focus (round 10).** (1) **Close `techniques_absent` in
writing.** Eighth deferral. Eight consecutive runs have answered every question
at subject grain; that is not a backlog any more, it is a decision nobody has
written down. Write the closure and stop carrying it. (2) **Say whether the
per-system routing count changed what you extracted, not just what you routed.**
This run computed the count at Phase 2d and it re-aimed the whole extraction pass
at one subsystem; a count computed and then ignored would score identically in
the depth cell. Make the row say which. (3) **When the apply seam turns out to
contain a defect, say so as its own outcome.** Two runs in three days have now
had a landing pay for itself by finding a bug at the seam it was applied to, and
both were recorded as ordinary `better` rows. That is a distinct and unusually
strong result — the corpus predicting a defect in a tree nobody was auditing —
and it is invisible in the ledger's vocabulary. **The check the next row should
make:** did a run report a seam-defect find, and did the fleet-reach number get
stated even when it was zero?
