...
(category `search-and-content`), one worker, expert draft first, then reconciled against the
...
**Techniques (6):** ctr-lifts-as-tie-breakers-not-a-stack, low-ctr-high-impression-rewrite-targets, mechanical-layer-only-never-the-copy, primary-keyword-triple-placement, structured-data-by-page-kind, title-and-meta-pixel-budget.

**Applications (3; stacks: node, prompt-pipeline):** node--structured-data-by-page-kind, node--title-and-meta-pixel-budget, prompt-pipeline--mechanical-layer-only-never-the-copy.
