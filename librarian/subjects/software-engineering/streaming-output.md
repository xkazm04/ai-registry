...
`sink-reversibility-decides-the-commit`. `emit-behind-the-revision-window` owns the companion axis and makes the assumption this corrects: it asserts irreversibility of **emission as such**, universally. Reversibility is a property of the SINK, and the interesting case is one producer feeding sinks that differ in it. The failure has a recognisable shape: the producer mutates the record IN BAND to satisfy the append-only sink - truncating, marking, redacting - and the whole cost lands on the other sink, which needed the record intact. So the clip is reported BESIDE the bytes rather than inside them, and reversibility is named per sink rather than assumed of emission. Two things worth carrying beyond this subject. **A measurement taken off an irreversible sink's contents is measuring every stage that ever mutated them**: the worker's first instrument would have reported 80,689 destroyed records instead of 222, because a redactor in the same sink was performing a second, independent in-band rewrite from a different stage - which also means that project's documented claim that those logs are the exact bytes its parser was handed is no longer true. **And the correct policy usually exists somewhere in the tree already**: here at exactly one site, sanitizing then re-checking that the result still parses and keeping the original if not, with a comment explaining that the unguarded form wipes a ledger. Known, written down, never generalized to the two sinks that needed it - which is the asymmetry tell this corpus keeps finding.
