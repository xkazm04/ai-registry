...
### 2026-09-23 - `/intake` ECC, one amendment + one application, shipped in-tree

`catalog-as-sync-key` argued normalization thoroughly and in one direction only. Everything it said was about normalizing too *little*: a digest that answers a question about the checkout, a false **diverged** that is wrong for everyone on one platform at once until the operators stop reading the field. The opposite direction was absent and is worse. A lossy canonicalizer, or a serialization that is not injective, maps two different inputs onto one digest - a false **in-sync**, which is the single state a sync key exists to make impossible, and which announces nothing.

Two independent instances, which is what made this convergence rather than an anecdote. The source ([[2026-09-23-ecc-agent-harness]]) recorded a canonicalizer that silently dropped own `__proto__` keys before hashing, so the chain built on it was authenticating a projection of the input; they took it as a breaking change with no dual-hash acceptance and no migration. The second instance is **this registry**, found by aiming the hunt at our own tree: `hashBundle` fed each file's relative path and then its bytes into one stream with no delimiter or length prefix, so the boundary was unrecoverable and `ab.md` holding "X" collided with `ab.m` holding "dX" at `sha256:f29ce5a2284c36dc`. Neither file is contrived; any edit moving a character across the name/content boundary produces it.
