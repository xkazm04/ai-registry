...
Source [[../../sources/2026-09-21-claude-code-from-source]]: an architecture book about an agent harness, read from the clone rather than the landing page. One amendment landed; the subject stays at 16 techniques.

- **`operator-tier-code-loading` widened from code to authority.** The technique's tier ladder, its absolute rule and its mechanical test were all stated over keys that *name code*. A runtime can pass that test - write a code-naming key through the untrusted tier, confirm it is ignored - and still hand the untrusted tier the same power through a key whose value a standing permission carve-out consumes: a write destination that skips approval, a read source trusted on content, an identity a policy matches. No code is named, so the tier check correctly answers no.
