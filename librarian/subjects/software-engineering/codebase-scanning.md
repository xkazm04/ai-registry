...
`causes-beside-the-finding-count`. `finding-lifecycle` deduplicates on two horizons and reads as exhaustive; both are **identity** horizons, and the axis this technique is about is a third. That sentence is what kept the gap invisible, and the golden path's own 'deduplicated, verified, prioritized work items' was the overclaim it produced - softened in this landing, because a list deduplicated on identity is not a work list. What makes the landing worth more than its rule: **the measurement refuted the rule's direction.** A collapse-by-cause is supposed to lower a total; here it raised it, from 76 to 77, because the one cause the consumer knew about had been used to delete its own seven rows and was then counted nowhere. Subtraction is not attribution. So the gap's direction is a diagnosis with two readings - more findings than causes means instances are being counted as work; more causes than the published number means a cause was discovered and counted nowhere. The live risk is over-collapsing, and it is bounded by exactly one thing: two findings that share a participant do not share a cause. In the measured backlog one artifact appeared against four others across four scopes, and collapsing on the shared participant would have destroyed four real findings. Where the stated relation is unavailable, the honest output is the finding count with 'cause relation not computed' beside it - a wrong collapse removes work while claiming precision.
