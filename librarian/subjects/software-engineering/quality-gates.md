...
### 2026-09-23 - `/intake` ECC, one technique + one correction

`unparseable-form-is-a-finding`, and it lands against a sentence this subject already had. `chokepoint-tag-registry` held that a negative-space source pattern match is defeated by any indirect form of the same access, and closed the question: "this is not a fixable weakness of the technique; it is the boundary of what a source scan can claim" - the stronger property purchasable only outside the source layer, at a linker, a package boundary or a capability-restricted runtime. That is right about the **matcher** and wrong about the **gate**, and the source ([[2026-09-23-ecc-agent-harness]]) demonstrates the difference: its env-documentation gate enumerates the access forms its own matcher cannot follow - destructuring, aliasing, spread, `Object.*` enumeration, `Reflect.*`, computed keys - and reddens the build when any of them appears, so the scan never has to see through an escape it has forbidden.

The asymmetry is the whole technique and it is why the hole is closable at all: **the target set belongs to the codebase and is unbounded; the escape set belongs to the grammar and is finite.** No finite pattern covers every name a codebase can invent, and the syntactic ways around a matcher are fixed by the language and can be written down beside the rule. What the gate gains is not a complete matcher but a closed world - either every access is in a form the check follows, or the check says so. That is `failure-not-empty-success` aimed at the gate's own reach, and `chokepoint-tag-registry` already makes exactly this move in the *positive* direction, where a renamed door must redden the gate as a broken instrument; only the negative direction was assumed unreachable.
