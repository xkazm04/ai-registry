...
3. Version bump — ONLY when you also edit SKILL.md to apply the improvement in the same change: minor (2.3.0 → 2.4.0) for a prompt/step refinement, major (2.x → 3.0.0) for a methodic redesign, patch for wording. Update the `version:` frontmatter field. Never bump without an applied edit; never edit the method without a bump.
4. Sync ritual (only when you bumped): (a) commit the skill directory as a STANDALONE commit on the current branch — message `skill(<name>): v<new> - <one-line reason>` — containing nothing but this skill's files (check `git ls-files` shows `SKILL.md` in the same casing as its siblings: `core.ignorecase` hides a lowercase `skill.md` that breaks discovery on the first case-sensitive clone); (b) propagate to the canonical copy this repo consumes the skill from (the registry, or `~/.claude/skills/<name>/`) — the METHOD only, never a repo's overlay. EXCEPTION: read `.personas/skill-registry.json` first — if the library already carries a HIGHER version than yours, do not overwrite it; keep your lesson in LESSONS.md and note the version conflict in the entry.
