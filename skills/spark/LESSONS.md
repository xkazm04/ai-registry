...
- **Compute the landing overlap at the go-gate, not at merge.** Phase 5 step 5 assumes the base checkout can take the merge. A package that fans out across many files (here a clause re-stamp over all 31 skills plus regenerated catalog/index) collided with a parallel session's uncommitted edits in four of them, discovered only after every gate was green. `git status --short` of the landing checkout intersected with the brief's file list is one command; when it is non-empty, the go-gate should name the landing path (land later / PR / operator commits first) so "Build now" is not silently "build, then block". Applied in 1.7.0 (Phase 4, before the gate), including the fan-out files no work package names.
- **The harness strips frontmatter when it loads a skill,** so a running agent cannot reliably see its own `version:`. Anything a skill must report about itself (version, name) should be resolved by a script from the lane file, not typed by the agent. Applied in tooling, not in this method: `scripts/log-run.mjs` resolves the version from the installation receipt, then the lane frontmatter.
- **Fetch the remote in Phase 0 and scout against it, not against the local base.** The spark hooked a shared clause that the other machine had already redesigned (scoped reflection: "no additional permission to edit another repository") 267 commits earlier on origin/main. The local design - write straight into the registry from every consumer - was built, gated green, and then had to be re-cut after the sync, when the operator asked to reconcile with the remote. `git fetch && git log HEAD..origin/<base> -- <target paths>` is one command; a non-empty result on the files the idea targets is a wave-1 constraint, not a merge chore. Applied in 1.7.0: Phase 0 step 4 fetches and reports the distance plus the inherited gate state, Phase 2 diffs the target paths against the remote.
