...
const SKIP_CALLEE_ROOT = new Set(['console', 'logger', 'log', 'debug', 'process', 'Sentry', 'posthog', 'analytics', 'z', 'path', 'fs', 'crypto', 'Buffer', 'JSON', 'Object', 'Array', 'Reflect', 'Math', 'Number', 'Date', 'Response', 'NextResponse']);
...
export function isProse(s, inDict) {
  ...
  // "Foundry — Gravitone", "Trust & compliance — Brand": a dash-joined title has no two
  // words side by side, and skipping it hid the one character several contracts ban
  return /\p{L}{2,}\s+\p{L}{2,}/u.test(t) || /\p{L}{2,}\s*[–—]\s*\p{L}{2,}/u.test(t);
}
