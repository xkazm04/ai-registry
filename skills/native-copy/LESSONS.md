...
## 1.2.1 - 2026-09-14 - personas-web

- **The review mixed truth into language quality.** Of 28 agreed findings on the money pages, 17 were escalated, 10 of them claims no EN-* rule can decide (tier names across pages, "nothing is ever sent to the cloud", an unsourced 94% test result, "Claude CLI" for a renamed product). The owner ruled on all of them in one line and said they are a fact-check notice, not translation or copy quality. Counted as findings they inflated the escalation list and the agreement figures of a review whose purpose is nativeness. Claims now go to a separate fact-check notice outside every count (1.3.0).
- **Applying the rulings showed why the notice must list keys, not topics.** "No tiers" reached 14 locale files, a dead `pricing.*` block nothing rendered, a decorative SVG painting three prices, the FAQ JSON-LD, and guide bodies whose "Builder tier" may name an interface mode rather than a plan. A topic-level note would have been applied to the visible cards only.
