...
- **Re-seated onto the lane's overlay contract (1.5.0 -> 1.6.0).** The body declared itself "personas-specific" and hardcoded one machine's absolute vault path in eight places, including Phase 0's bootstrap and every "files updated" printout. On a second machine that root does not exist, so the memory loop the whole skill is built around wrote nowhere. The vault now resolves from an overlay candidate list and falls back to `<repo>/.research/`, which Phase 0 creates.
